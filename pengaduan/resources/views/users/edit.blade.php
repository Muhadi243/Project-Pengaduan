...
@error('username') is-invalid @enderror" id="exampleInputName"
placeholder="Nama lengkap" name="username" value="{{$user->username ??
old('username')}}">
 @error('username') <span class="textdanger">{{$message}}</span> @enderror
 </div>
...
'admin' || old('level') == 'admin')selected @endif>Admin</option>
 <option value="petugas" @if($user->level
== 'petugas' || old('level') == 'petugas')selected
@endif>Petugas</option>
 <option value="masyarakat" @if($user->level ==
'masyarakat' || old('level') == 'masyarakat')selected @endif>Masyarakat</option>
